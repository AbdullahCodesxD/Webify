...
const saveFunction = function () {
  ...
  if (currTool === "other") {
    // Passing the other object from state inside the function
    const other = model.state.toolsInfo.other;
    const newCSS = editorView.updateOthers(model.updateOthers, other);
    console.log(newCSS);
    editorView.addCSS(newCSS);
  }
  if (currTool === "img") {
    editorView.checkImages();
