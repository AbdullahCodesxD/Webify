...
export const updateOthers = function (other, css) {
  // ----------------------------------------------------------------
  const CSSArr = css.split("}");
  // Add / Update the font-size of link
  // Getting the font-size
  const linkSize = other[0]["a_font-size"];
  // Making a font-size String
  const linkSizeString =
    linkSize === "" ? "" : `a{font-size:${linkSize}px !important;}`;
  // Checking if css already contains a

  const indexOfA = CSSArr.includes(" a");
  const indexOfA2 = CSSArr.includes("a{");
  console.log(indexOfA, indexOfA2);
  if (indexOfA >= 0 || indexOfA2 >= 0) {
    const index = indexOfA || indexOfA2;
  ...
  // ----------------------------------------------------------------
  // Add / update the margin of section
  const sectionMargin = other[1]["section_margin"];
  const sectionMarginString =
    sectionMargin === "" ? "" : `margin:${sectionMargin}px 0 !important;`;

  // ----------------------------------------------------------------
  // Add/ update the padding of section
  const sectionPadding = other[2]["section_padding"];
  const sectionPaddingString =
    sectionPadding === "" ? "" : `padding:${sectionPadding}px !important;`;
  // ----------------------------------------------------------------
  // Add / update the width of section
  const sectionWidth = other[3]["section_width"];
  const sectionWidthString =
    sectionWidth === "" ? "" : `width:${sectionWidth}px !important;  `;

  // Combining the css of section
  const sectionString = `section{${
    sectionMarginString + sectionPaddingString + sectionWidthString
  }};`;

  // ----------------------------------------------------------------
  // Returning the values from section
  return [linkSizeString, sectionString];
};
// Update images
